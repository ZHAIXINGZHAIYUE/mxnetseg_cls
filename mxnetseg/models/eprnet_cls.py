    ...
    def __init__(
        ...
        **kwargs
        ...
        with self.name_scope():
            ...
            self.drop = nn.Dropout(drop) if drop > 0.0 else None
...
class _MPUnit(nn.HybridBlock):
    def __init__(
        ...
        **kwargs
        ...
        with self.name_scope():
            self.concurrent = (
                HybridConcurrent(axis=1) if not light else HybridConcurrentSum()
            )
...
def EPRNET_CLS(pretrained=False, **kwargs):
    return get_eprnet_cls(
        False, pretrained, ckpt_name="eprnetcls_imagenet.params", **kwargs
    )
...
def EPRNET_CLS_LIGHT(pretrained=False, **kwargs):
    return get_eprnet_cls(
        True, pretrained, ckpt_name="eprnetcls_light_imagenet.params", **kwargs
    )
